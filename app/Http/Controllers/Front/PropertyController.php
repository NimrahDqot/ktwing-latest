...
class PropertyController extends Controller
{
    ...
    public function category_detail($slug)
    {
    	...
    	$property_items = Property::with('rPropertyCategory','rPropertyLocation')->where(['property_category_id'=>$property_category_detail->id, 'is_approved'=>'1','property_status'=>'Active'])->paginate(15);
    ...
    public function location_detail($slug)
    {
        ...
        $property_items = Property::with('rPropertyCategory','rPropertyLocation')->where(['property_location_id'=>$property_location_detail->id, 'is_approved'=>'1','property_status'=>'Active'])->paginate(15);
    ...
    public function search_property_result(Request $request){
        ...
        if($request->construction_property_status){
            $properties = $properties->where('construction_property_status', $request->construction_property_status);
        }
    ...
    public function wishlist_add($id)
    {
	    ...
	    $check_previous = Wishlist::where('user_id',Auth::id())->where('property_id',$id)->first();

	    if($check_previous) {
            $check_previous->delete();
    ...
    public function ajax_wishlist_add($id)
    {
	    ...
	    if($check_previous) {
            $check_previous->delete();
            return response()->json(['is_success' => true, 'message' => SUCCESS_ITEM_REMOVED_FROM_WISHLIST]);
        }
	    $user_data = Auth::user();
        $obj = new Wishlist;
        $obj->user_id = $user_data->id;
