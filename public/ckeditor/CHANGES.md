...
* [#4783](https://github.com/ckeditor/ckeditor4/issues/4783): Fixed: The [Accessibility Help](https://ckeditor.com/cke4/addon/a11yhelp) dialog does not contain info about focus being moved{{ ADD_NEW }}to the editing area upon activating a toolbar button.
...
* [#4782](https://github.com/ckeditor/ckeditor4/issues/4782): [Firefox] Fixed: `TypeError` is thrown when switching to Source View and{{ ADD_NEW }}while [Autocomplete](https://ckeditor.com/cke4/addon/autocomplete) plugin is enabled.
...
* Fixed XSS vulnerability in the HTML data processor reported by [Michał Bentkowski](https://twitter.com/securitymb) of Securitum.
	...
	Issue summary: It was possible to execute XSS inside CKEditor after persuading the victim to: (i) switch CKEditor to source mode, then (ii) paste a specially crafted HTML code, prepared by the attacker, into the opened CKEditor source area, and (iii) switch{{ ADD_NEW }}to WYSIWYG mode or (i) copy the specially crafted HTML code, prepared by the attacker and (ii) paste it into CKEditor in WYSIWYG mode.
...
* Fixed XSS vulnerability in the WebSpellChecker Dialog plugin reported by [Pham Van Khanh](https://twitter.com/rskvp93) from Viettel Cyber Security.
	...
	Issue summary: It was possible to execute XSS using CKEditor after persuading the victim to: (i) switch CKEditor to source mode, then (ii) paste a specially crafted HTML code, prepared by the attacker, into the opened CKEditor source area, then (iii) switch{{ ADD_NEW }}to WYSIWYG mode, and (iv) preview CKEditor content outside CKEditor editable area.
...
* Fixed XSS vulnerability in the HTML parser reported by [maxarr](https://hackerone.com/maxarr).
	...
	Issue summary: It was possible to execute XSS inside CKEditor after persuading the victim to: (i) switch CKEditor to source mode, then (ii) paste a specially crafted HTML code, prepared by the attacker, into the opened CKEditor source area, and (iii) switch{{ ADD_NEW }}to WYSIWYG mode.
...
* [#11788](https://dev.ckeditor.com/ticket/11788): Fixed: It is not possible to change the language{{ ADD_NEW }}to _Not set_ in the [Code Snippet](https://ckeditor.com/cke4/addon/codesnippet) dialog window.
...
* [#9940](https://dev.ckeditor.com/ticket/9940): [Firefox] Navigating{{ ADD_NEW }}to a page with the editor was making the entire page editable.
