					...
					<div class="inner">
						...
						<form action="{{ route('customer_login_store') }}" method="post">
							...
							<div class="form-group">
								...
								<input type="email" class="form-control" name="email" value="{{ (env('PROJECT_MODE') == 0) ? 'customer@gmail.com' : '' }}">
							</div>
							<div class="form-group">
								<label for="">{{ PASSWORD }}</label>
								<input type="password" class="form-control" name="password" value="{{ (env('PROJECT_MODE') == 0) ? '1234' : '' }}">
							</div>
