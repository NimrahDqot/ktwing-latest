...
@if ($errors->any())
	...
	@endforeach
	<script>Swal.fire({icon: 'error',title: '',html: '{ $all_error) !!}'})</script>
@endif

@if(session()->get('error'))
	<script>Swal.fire({icon: 'error',title: '',html: '{ session()->get('error')) !!}'})</script>
@endif

@if(session()->get('success'))
	<script>Swal.fire({icon: 'success',title: '',html: '{ session()->get('success')) !!}'})</script>
@endif
